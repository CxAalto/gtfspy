...
DEFAULT_STOP_TO_STOP_LINK_ATTRIBUTES = ["n_vehicles", "duration_avg", "d", "route_I_counts"]
...
def stop_to_stop_network_for_route_type(
    gtfs, route_type, link_attributes=None, start_time_ut=None, end_time_ut=None
):
    """
    ...
    events_df = gtfs.get_transit_events(
        start_time_ut=start_time_ut, end_time_ut=end_time_ut, route_type=route_type
    )
    if len(net.nodes()) < 2:
    ...
    for key, link_events in link_event_groups:
        ...
        if link_attributes is None:
            ...
            if "capacity_estimate" in link_attributes:
                link_data["capacity_estimate"] = route_types.ROUTE_TYPE_TO_APPROXIMATE_CAPACITY[
                    route_type
            ...
            if "distance_shape" in link_attributes:
                ...
                if found is None:
                    ...
                    distance = gtfs.get_shape_distance_between_stops(
                        link_event["trip_I"], int(link_event["from_seq"]), int(link_event["to_seq"])
                    )
...
def combined_stop_to_stop_transit_network(gtfs, start_time_ut=None, end_time_ut=None):
    ...
    for route_type in route_types.TRANSIT_ROUTE_TYPES:
        graph = stop_to_stop_network_for_route_type(
            gtfs, route_type, start_time_ut=start_time_ut, end_time_ut=end_time_ut
        )
...
def temporal_network(gtfs, start_time_ut=None, end_time_ut=None, route_type=None):
    """
    ...
    events_df.rename(columns={"from_seq": "seq"}, inplace=True)
    return events_df
...
def route_to_route_network(gtfs, walking_threshold, start_time, end_time):
    ...
    for i in routes.itertuples():
        graph.add_node(
            i.route_id, attr_dict={"type": i.type, "color": route_types.ROUTE_TYPE_TO_COLOR[i.type]}
        )
