...
class TestImport(unittest.TestCase):
    ...
    def test_calendarDatesLoader(self):
        ...
        rows = self.conn.execute(
            "SELECT * " "FROM calendar " "WHERE service_id='phantom_service'"
        ).fetchall()
    ...
    def test_dayLoader(self):
        ...
        self.assertEqual(
            len(self.conn.execute(query2).fetchall()),
            6,
            "There should be 6 days with the trip_I " "corresponding to service_id service1",
        )
        query3 = (
            "SELECT * "
            "FROM days "
        ...
        self.assertEqual(
            len(self.conn.execute(query3).fetchall()),
            0,
            "There should be no trip on date 2016-03-22" "for service1 due to calendar_dates",
        )
        query4 = (
            "SELECT date "
            "FROM days "
        ...
        self.assertEqual(
            len(self.conn.execute(query4).fetchall()),
            1,
            "There should be only one trip for service 2",
        )
        self.assertEqual(
            self.conn.execute(query4).fetchone()["date"],
            "2016-03-22",
        ...
        self.assertEqual(
            len(res),
            0,
            "there should be no phantom trips due to phantom service"
    ...
    def test_stopDistancesLoader(self):
        ...
        for row in rows:
            ...
            assert (
                row["d"] >= 0
            ), "distance should be defined for all pairs in the stop_distances table"
    ...
    def test_frequencyLoader(self):
        ...
        count = self.conn.execute(
            "SELECT count(*) AS count FROM trips JOIN calendar "
            "USING(service_I) WHERE service_id='freq_service'"
        ).fetchone()["count"]

        assert count == 12, count
        rows = self.conn.execute(
            "SELECT trip_I FROM trips JOIN calendar "
            "USING(service_I) WHERE service_id='freq_service'"
        ).fetchall()
        for row in rows:
            trip_I = row["trip_I"]
            res = self.conn.execute(
                "SELECT * FROM stop_times WHERE trip_I={trip_I}".format(trip_I=trip_I)
            ).fetchall()
    ...
    def test_transfersLoader(self):
        ...
        no_transfer_rows = self.conn.execute(
            base_query, (from_stop_I_no_transfer, to_stop_I_no_transfer)
        ).fetchall()
        assert len(no_transfer_rows) == 0
        timed_transfer_rows = self.conn.execute(
            base_query, (from_stop_I_timed_transfer, to_stop_I_timed_transfer)
        ).fetchall()
        assert len(timed_transfer_rows) == 1
        assert timed_transfer_rows[0]["min_transfer_time"] == 0
        min_transfer_rows = self.conn.execute(
            base_query, (from_stop_I_min_transfer, to_stop_I_min_transfer)
        ).fetchall()
    ...
    def test_importMultiple(self):
        ...
        rows = self.conn.execute(
            "SELECT count(*) FROM calendar_dates GROUP BY service_I"
        ).fetchall()
        for row in rows:
            self.assertIs(row[0], 1)
    ...
    def test_resequencing_stop_times(self):
        ...
        gtfs_source["stop_times.txt"] = self.stopTimesText = (
            "trip_id, arrival_time, departure_time, stop_sequence, stop_id"
            "\nservice1_trip1,0:06:10,0:06:10,0,SID1"
