...
def create_stops_geojson_dict(gtfs, fields=None):
    ...
    for i, node_tuple in enumerate(nodes.itertuples()):
        feature = {
            "type": "Feature",
            "id": str(i),
            "geometry": {"type": "Point", "coordinates": [node_tuple.lon, node_tuple.lat]},
            "properties": {"stop_I": str(node_tuple.stop_I), "name": node_tuple.name},
        }
...
def write_temporal_networks_by_route_type(gtfs, extract_output_dir):
    ...
    for route_type in route_types.TRANSIT_ROUTE_TYPES:
        pandas_data_frame = temporal_network(
            gtfs, start_time_ut=None, end_time_ut=None, route_type=route_type
        )
        tag = route_types.ROUTE_TYPE_TO_LOWERCASE_TAG[route_type]
...
def create_sections_geojson_dict(G, start_time_ut=None, end_time_ut=None):
    multi_di_graph = combined_stop_to_stop_transit_network(
        G, start_time_ut=start_time_ut, end_time_ut=end_time_ut
    )
...
def create_routes_geojson_dict(G):
    ...
    for routeShape in G.get_all_route_shapes(use_shapes=False):
        ...
        feature["geometry"] = geometry
        properties = {
            "route_type": int(routeShape["type"]),
...
def __replace_I_with_id(
    ...
    merged = pandas.merge(
        current_table, from_table, how="left", left_on=old_column_current, right_on=old_column_from
    )
...
def _write_gtfs_calendar_dates(gtfs, output_file):
    ...
    __replace_I_with_id(
        gtfs, calendar_dates_table, "calendar", "service_I", "service_I", "service_id"
    )
...
def _write_gtfs_frequencies(gtfs, output_file):
    raise NotImplementedError(
        "Frequencies should not be outputted from GTFS as they are included in other tables."
    )
...
def _write_gtfs_transfers(gtfs, output_file):
    ...
    __replace_I_with_id(
        gtfs, transfers_table, "stops", "from_stop_I", "stop_I", "stop_id", "from_stop_id"
    )
    __replace_I_with_id(
        gtfs, transfers_table, "stops", "to_stop_I", "stop_I", "stop_id", "to_stop_id"
    )
...
def _write_gtfs_stop_distances(gtfs, output_file):
    ...
    __replace_I_with_id(
        gtfs, stop_distances, "stops", "from_stop_I", "stop_I", "stop_id", "from_stop_id"
    )
    __replace_I_with_id(
        gtfs, stop_distances, "stops", "to_stop_I", "stop_I", "stop_id", "to_stop_id"
    )
...
def main():
    ...
    parser_routingnets.add_argument(
        "basename", help="Basename for the output files"
    )  # Parsing copy
