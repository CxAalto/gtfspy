...
class FrequenciesLoader(TableLoader):
        ...
        for reader, prefix in zip(readers, prefixes):
            for row in reader:
                yield dict(
                    ...
                    exact_times=int(row["exact_times"])
                    if "exact_times" in row and row["exact_times"].isdigit()
                    else 0,
                )
    ...
    def post_import(self, cur):
        ...
        for freq_tuple in frequencies_df.itertuples():
            trip_data = pandas.read_sql_query(
                "SELECT * FROM trips WHERE trip_I= " + str(int(freq_tuple.trip_I)), conn
            )
            ...
            trip_duration = cur.execute(
                "SELECT max(arr_time_ds) - min(dep_time_ds) "
                "FROM stop_times "
                "WHERE trip_I={trip_I}".format(trip_I=str(int(freq_tuple.trip_I)))
            ).fetchone()[0]
            if trip_duration is None:
                raise ValueError(
                    "Stop times for frequency trip "
                    + trip_data.trip_id
            ...
            for i, start_time in enumerate(start_times_ds):
                ...
                query = (
                    "INSERT INTO trips (trip_id, route_I, service_I, shape_id, direction_id, "
                ...
                for seq, (dep_time_ds, arr_time_ds, shape_break, stop_I) in enumerate(
                    zip(dep_times_ds, arr_times_ds, shape_breaks, stop_Is)
                ):
                    arr_time_hour = int(arr_time_ds // 3600)
                    query = (
                        "INSERT INTO stop_times (trip_I, stop_I, arr_time, "
        ...
        for trip_I in trip_Is:
            for table in ["trips", "stop_times"]:
                cur.execute(
                    "DELETE FROM {table} WHERE trip_I={trip_I}".format(table=table, trip_I=trip_I)
                )
        self._conn.commit()
