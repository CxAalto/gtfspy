...
class RouteLoader(TableLoader):
    ...
    extra_values = [
        "(SELECT agency_I FROM agencies WHERE agency_id=:_agency_id )",
    ]
        ...
        for reader, prefix in zip(readers, prefixes):
            for row in reader:
                ...
                yield dict(
                    ...
                    _agency_id=prefix + decode_six(row["agency_id"])
                    if "agency_id" in row
                    else None,
                    ...
                    text_color=decode_six(row["route_text_color"])
