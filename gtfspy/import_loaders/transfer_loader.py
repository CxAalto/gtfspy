...
class TransfersLoader(TableLoader):
    ...
    tabledef = (
        "(from_stop_I INT, " "to_stop_I INT, " "transfer_type INT, " "min_transfer_time INT" ")"
    )
    extra_keys = [
        "from_stop_I",
        "to_stop_I",
    ...
    extra_values = [
        "(SELECT stop_I FROM stops WHERE stop_id=:_from_stop_id)",
        ...
        for reader, prefix in zip(readers, prefixes):
            for row in reader:
                ...
                yield dict(
                    ...
                    if ("min_transfer_time" in row and (row.get("min_transfer_time").strip()))
                    else None,
                )
