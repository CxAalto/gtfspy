...
class StopTimesLoader(TableLoader):
    ...
    table = "stop_times"
    tabledef = (
        "(stop_I INT, trip_I INT, arr_time TEXT, dep_time TEXT, "
    ...
    extra_values = [
        "(SELECT stop_I FROM stops WHERE stop_id=:_stop_id )",
    ...
    def gen_rows(self, readers, prefixes):
        for reader, prefix in zip(readers, prefixes):
            for row in reader:
                ...
                assert (
                    row["arrival_time"] != ""
                ), "Some stop_times entries is missing arrival time information."
                assert (
                    row["departure_time"] != ""
                ), "Some stop_times entries is missing departure time information."
                assert (
                    row["stop_sequence"] != ""
                ), "Some stop_times entries is missing seq information."
                assert (
                    row["stop_id"] != ""
                ), "Some stop_times entries is missing stop_id information."
                assert (
                    row["trip_id"] != ""
                ), "Some stop_times entries is missing trip_id information."
                yield dict(
    ...
    def index(cls, cur):
        ...
        cur.execute(
            "CREATE INDEX idx_stop_times_ath_tid_sid ON stop_times (arr_time_hour, trip_I, stop_I)"
        )
...
def resequence_stop_times_seq_values(conn):
    cursor = conn.cursor()
    rows = cursor.execute(
        "SELECT ROWID, trip_I, seq FROM stop_times ORDER BY trip_I, seq"
    ).fetchall()
    old_trip_I = ""
    correct_seq = 1
    for row in rows:
...
def calculate_trip_shape_breakpoints(conn):
    ...
    trip_Is = [x[0] for x in cur.execute("SELECT DISTINCT trip_I FROM stop_times").fetchall()]
    for trip_I in trip_Is:
        ...
        stop_points = [
            dict(seq=row[0], lat=row[1], lon=row[2], stop_I=row[3])
            for row in cur
            if row[1] and row[2]
        ]
        ...
        if cache_key in breakpoints_cache:
            ...
            breakpoints, badness = shapes.find_segments(stop_points, shape_points)
        ...
        cur.executemany(
            "UPDATE stop_times SET shape_break=? " "WHERE trip_I=? AND seq=? ",
            (
