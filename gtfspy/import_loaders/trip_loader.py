...
class TripLoader(TableLoader):
    ...
    tabledef = (
        "(trip_I INTEGER PRIMARY KEY, trip_id TEXT UNIQUE NOT NULL, "
        ...
        for reader, prefix in zip(readers, prefixes):
            for row in reader:
                ...
                yield dict(
                    ...
                    direction_id=decode_six(row["direction_id"])
...
def update_trip_travel_times_ds(conn):
    ...
    cur = conn.cursor()
    cur0.execute(
        """SELECT trip_I, min(dep_time), max(arr_time)
    ...
    cur.executemany(
        """UPDATE trips SET start_time_ds=?, end_time_ds=? WHERE trip_I=?""", iter_rows(cur0)
    )
    conn.commit()
