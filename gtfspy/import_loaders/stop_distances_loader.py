...
class StopDistancesLoader(TableLoader):
    ...
    def post_import(self, cur):
        ...
        stmt = (
            "SELECT min_transfer_time, from_stop_I, to_stop_I "
            "FROM transfers "
        ...
        cur.execute(stmt)
        cur2.executemany(
            "INSERT OR IGNORE INTO stop_distances "
        ...
        cur.execute(stmt)
        cur2.executemany(
            "INSERT OR IGNORE INTO stop_distances " "(from_stop_I, to_stop_I) " "VALUES (?,?)", cur
        )
        cur.execute(stmt)
        cur2.executemany(
            "UPDATE stop_distances "
        ...
        cur = conn.cursor()
        cur2 = conn.cursor()
        cur.execute("SELECT from_stop_I, to_stop_I " "FROM transfers " "WHERE transfer_type=3")
        cur2.executemany("DELETE FROM stop_distances " "WHERE from_stop_I=? and to_stop_I=?", cur)
        conn.commit()
    ...
    def export_stop_distances(self, conn, f_out):
        ...
        f_out.write("#from_stop_I,to_stop_I," "lat1,lon1,lat2,lon2," "d,min_transfer_time\n")
        for row in cur:
