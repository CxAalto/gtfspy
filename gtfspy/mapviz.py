...
def plot_route_network_from_gtfs(
    g,
    ax=None,
    spatial_bounds=None,
    map_alpha=0.8,
...
def plot_as_routes(
    ...
    bound_pixel_xs, bound_pixel_ys = smopy_map.to_pixels(
        numpy.array([lat_min, lat_max]), numpy.array([lon_min, lon_max])
    )
    ...
    if legend:
        ...
        labels = [
            ROUTE_TYPE_TO_SHORT_DESCRIPTION[route_type]
            for route_type in route_types_to_lines.keys()
        ]
...
def plot_routes_as_stop_to_stop_network(
    from_lats,
    from_lons,
    to_lats,
    to_lons,
    ...
    for from_lat, from_lon, to_lat, to_lon, attribute, color_attribute, zorder, line_label in zip(
        from_lats, from_lons, to_lats, to_lons, attributes, color_attributes, zorders, line_labels
    ):
        ...
        xs, ys = smopy_map.to_pixels(
            numpy.array([from_lat, to_lat]), numpy.array([from_lon, to_lon])
        )
        ...
        ax.plot(
            ...
            ys,
            color=color,
            linewidth=attribute * linewidth_multiplier,
            zorder=zorder,
            alpha=alpha,
        )
        if line_label:
...
def _expand_spatial_bounds_to_fit_axes(bounds, ax_width, ax_height):
    ...
    if x_per_y_axes > x_per_y_meters:  # x-axis
        ...
        mean_lon = (b["lon_min"] + b["lon_max"]) / 2.0
        lon_min = mean_lon - d_lon_new / 2.0
        lon_max = mean_lon + d_lon_new / 2.0
        spatial_bounds = {
            "lon_min": lon_min,
            "lon_max": lon_max,
            "lat_min": b["lat_min"],
        ...
        mean_lat = (b["lat_min"] + b["lat_max"]) / 2.0
        lat_min = mean_lat - d_lat_new / 2.0
        lat_max = mean_lat + d_lat_new / 2.0
        spatial_bounds = {
            "lon_min": b["lon_min"],
            "lon_max": b["lon_max"],
            "lat_min": lat_min,
...
def plot_route_network_thumbnail(g, map_style=None):
    ...
    return plot_route_network_from_gtfs(
        g, ax, spatial_bounds, map_alpha=1.0, scalebar=False, legend=False, map_style=map_style
    )


def plot_stops_with_categorical_attributes(
    lats_list,
    lons_list,
    attributes_list,
    s=0.5,
    ...
    axes = []
    for lats, lons, attributes, c in zip(
        lats_list, lons_list, attributes_list, mcolors.BASE_COLORS
    ):
...
def plot_stops_with_attributes(
    lats,
    lons,
    attribute,
    s=0.5,
    ...
    if args not in get_smopy_map.maps:
        ...
        try:
            get_smopy_map.maps[args] = smopy.Map(
                (lat_min, lon_min, lat_max, lon_max), tileserver=tileserver, **kwargs
            )
        except URLError:
            raise RuntimeError(
