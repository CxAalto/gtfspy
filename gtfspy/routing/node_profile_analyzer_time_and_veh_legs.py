...
class NodeProfileAnalyzerTimeAndVehLegs:
    ...
    def from_profile(cls, node_profile, start_time_dep, end_time_dep):
        ...
        return NodeProfileAnalyzerTimeAndVehLegs(
    ...
    def __init__(self, labels, walk_to_target_duration, start_time_dep, end_time_dep):
        ...
        self.all_labels = [
            ...
            if (start_time_dep <= label.departure_time <= end_time_dep)
        ]
        after_label_candidates = [
            label
            for label in self._node_profile_final_labels
    ...
    def get_time_profile_analyzer(self, max_n_boardings=None):
        ...
        if not max_n_boardings in self._n_boardings_to_simple_time_analyzers:
            if max_n_boardings == 0:
                ...
                candidate_labels = [
                    LabelTimeSimple(label.departure_time, label.arrival_time_target)
                    for label in self._node_profile_final_labels
                    if (
    ...
    def n_boardings_on_fastest_trip(self):
        return min(
            self._labels_within_time_frame,
            key=lambda label: label.arrival_time_target - label.departure_time,
        ).n_boardings
    ...
    def median_temporal_distances(self, min_n_boardings=None, max_n_boardings=None):
        ...
        median_temporal_distances = [
            float("inf") for _ in range(min_n_boardings, max_n_boardings + 1)
        ]
    ...
    def _get_colors_for_boardings(cls, min_n_boardings, max_n_boardings):
        ...
        colors = [
            cmap(float(n_boardings) / max_n_boardings)
            for n_boardings in range(int(max_n_boardings) + 1)
        ]
    ...
    def _get_fill_and_line_colors(self, min_n, max_n):
        ...
        for n, color_tuple in nboardings_to_color.items():
            c = NodeProfileAnalyzerTimeAndVehLegs._multiply_color_saturation(
                color_tuple, line_saturation_multiplier
            )
    ...
    def plot_new_transfer_temporal_distance_profile(
        self,
        timezone=None,
        ...
        if walk_duration < float("inf"):
            ...
            ax.plot(
                ...
                lw=n_boardings_to_lw[0],
                color=n_boardings_to_line_color[0],
            )
            ...
            for i, journey_label in enumerate(labels):
                ...
                for block in prev_profile_block_analyzer._profile_blocks:
                    if block.distance_end != block.distance_start:
                        ...
                        prev_dep_time = max(
                            prev_dep_time, journey_label.departure_time - waiting_time
                        )
                # prev dep time is now known
                waiting_time = journey_label.departure_time - prev_dep_time
                lw = n_boardings_to_lw[n_boardings]
                xs = [
                    _ut_to_unloc_datetime(prev_dep_time),
                    _ut_to_unloc_datetime(journey_label.departure_time),
                ]
                ys = (
                    numpy.array([journey_label.duration() + waiting_time, journey_label.duration()])
                    / duration_divider
                )
                max_tdist = max(ys[0], max_tdist)
                ax.plot(xs, ys, color=n_boardings_to_line_color[n_boardings], lw=lw)  # "k",
        ...
        if highlight_fastest_path:
            ...
            (
                ...
                fastest_path_time_analyzer.profile_block_analyzer.get_vlines_and_slopes_for_plotting()
            )
            lw = fastest_path_lw
            ls = "--"
            for vline in vlines:
                ax.plot(
                    [_ut_to_unloc_datetime(x) for x in vline["x"]],
                    numpy.array(vline["y"]) / duration_divider,
                    ls=":",
                    lw=lw / 2,
                    color="k",
                )
            for slope in slopes:
                ax.plot(
                    [_ut_to_unloc_datetime(x) for x in slope["x"]],
                    numpy.array(slope["y"]) / duration_divider,
                    ls=ls,
                    color="k",
                    lw=lw,
                )
            ...
            for (x, y), letter in zip(journeys, journey_letters):
                if x < _ut_to_unloc_datetime(self.end_time_dep):
                    ...
                    ax.text(
                        x
                        + datetime.timedelta(
                            seconds=(self.end_time_dep - self.start_time_dep) / 40.0
                        ),
                        y,
                        letter,
                        va="center",
    ...
    def plot_temporal_distance_pdf_horizontal(
        ...
        assert numpy.isclose(
            walking_is_fastest_time + non_walk_blocks_total_time,
        ...
        for n_boardings in range(min_n_boardings, max_n_boardings + 1):
            ...
            for block_now in blocks_now:
                first_index = numpy.searchsorted(
                    distance_split_points_ordered, block_now.distance_end
                )
                last_index = numpy.searchsorted(
                    distance_split_points_ordered, block_now.distance_start
                )
        ...
        if walking_is_fastest_time > 0:
            text = "$P(\\mathrm{walk}) = " + (
                "%.2f$" % (walking_is_fastest_time / (self.end_time_dep - self.start_time_dep))
            )
            ax.plot(
                [0, 10],
                [
                    self._walk_to_target_duration / duration_divider,
                    self._walk_to_target_duration / duration_divider,
                ],
                color=line_colors[0],
                lw=5,
        ...
        for n_boardings in range(max(1, min_n_boardings), max_n_boardings + 1):
            ...
            ax.fill_betweenx(
                ...
                label=label,
                color=fill_colors[n_boardings],
                zorder=n_boardings,
            )
            ax.plot(
                pdf_values_to_plot * duration_divider,
                temporal_distance_values_to_plot / duration_divider,
                color=line_colors[n_boardings],
                zorder=n_boardings,
            )
    ...
    def plot_fastest_temporal_distance_profile(self, timezone=None, **kwargs):
        ...
        fig = npat.plot_temporal_distance_profile(timezone=timezone, **kwargs)
        return fig
    ...
    def get_node_profile_measures_as_dict(self):
        ...
        profile_measure_dict = {
            key: value(self)
            for key, value in zip(profile_observable_names, profile_summary_methods)
        }
