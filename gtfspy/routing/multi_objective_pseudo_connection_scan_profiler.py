...
class MultiObjectivePseudoCSAProfiler(AbstractRoutingAlgorithm):
    ...
    def _add_pseudo_connection_departures_to_stop_departure_times(self):
        ...
        for key, value in self._stop_departure_times_with_pseudo_connections.items():
            self._stop_departure_times_with_pseudo_connections[key] = set(value)
        for pseudo_connection in self._pseudo_connections:
            ...
            self._stop_departure_times_with_pseudo_connections[pseudo_connection.departure_stop]\
                .add(pseudo_connection.departure_time)
        for key, value in self._stop_departure_times_with_pseudo_connections.items():
    ...
    def __initialize_node_profiles(self):
        ...
        for node in self._all_nodes:
            if self._walk_network.has_edge(self._target_stop, node):
                edge_data = self._walk_network.get_edge_data(self._target_stop, node)
                walk_duration_to_target = edge_data["d_walk"] / float(self._walk_speed)
            elif node == self._target_stop:
            ...
            self._stop_profiles[node] = NodeProfileMultiObjective(dep_times=self._stop_departure_times_with_pseudo_connections[node],
                                                                  ...
                                                                  connection_dep_times=self._stop_departure_times[node])
    @timeit
    def __compute_stop_dep_and_arrival_times(self):
        stop_departure_times = defaultdict(lambda: set())
        stop_arrival_times = defaultdict(lambda: set())
        for connection in self._transit_connections:
            stop_arrival_times[connection.arrival_stop].add(connection.arrival_time)
            stop_departure_times[connection.departure_stop].add(connection.departure_time)
        for stop in stop_departure_times:
            stop_departure_times[stop] = numpy.array(sorted(list(stop_departure_times[stop])))
        for stop in stop_arrival_times:
            stop_arrival_times[stop] = numpy.array(sorted(list(stop_arrival_times[stop])))
    ...
    def _run(self):
        ...
        for i, connection in enumerate(self._all_connections):
            ...
            arrival_node_labels_modified = self._copy_and_modify_labels(arrival_node_labels_orig,
                                                               connection.departure_time,
                                                               increment_vehicle_count=increment_vehicle_count)
            arrival_node_labels_modified = set(compute_pareto_front(list(arrival_node_labels_modified)))
            ...
            all_pareto_optimal_labels = merge_pareto_frontiers(arrival_node_labels_modified, trip_labels)
