...
class TemporalProfileBlockAnalyzer:
    ...
    def _temporal_distance_cdf(self):
        ...
        for block in self._profile_blocks:
            if block.distance_start == float('inf'):
                infinity_waiting_time += block.width()
            elif block.distance_start == block.distance_end == self._cutoff_distance:
                cutoff_total_time += block.width()
            else:
        ...
        assert (numpy.isclose(
            [unnormalized_cdf[-1]],
            [self._end_time - self._start_time - infinity_waiting_time - cutoff_total_time]
        ).all())

        if cutoff_total_time > 0:
            index = numpy.nonzero(distance_split_points_ordered == self._cutoff_distance)[0][0]
            ...
            unnormalized_cdf[(index + 1):] = unnormalized_cdf[(index + 1):] + cutoff_total_time
