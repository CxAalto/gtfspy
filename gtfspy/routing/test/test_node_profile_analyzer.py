...
class TestNodeProfileAnalyzer(TestCase):
    ...
    def test_trip_duration_statistics_simple(self):
        pairs = [
            LabelTime(departure_time=1, arrival_time_target=2),
            LabelTime(departure_time=2, arrival_time_target=4),
            LabelTime(departure_time=4, arrival_time_target=5)
        ]
        profile = NodeProfileNaive()
        for pair in pairs:
    ...
    def test_temporal_distance_statistics_with_walk(self):
        pt1 = LabelTime(departure_time=1, arrival_time_target=2)
        pt2 = LabelTime(departure_time=4, arrival_time_target=5)   # not taken into account by the analyzer
        profile = NodeProfileNaive(1.5)
        assert isinstance(pt1, LabelTime), type(pt1)
    ...
    def test_all_plots(self):
        ...
        profile = NodeProfileNaive()
        profile.update_pareto_optimal_tuples(LabelTime(departure_time=2 * 60, arrival_time_target=3 * 60))
        profile.update_pareto_optimal_tuples(LabelTime(departure_time=4 * 60, arrival_time_target=25 * 60))
