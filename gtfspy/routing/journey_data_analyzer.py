...
class JourneyDataAnalyzer:
    ...
    def get_journey_legs_to_target(
        self,
        target,
        fastest_path=True,
        min_boardings=False,
        all_leg_sections=True,
        ignore_walk=False,
        diff_threshold=None,
        diff_path=None,
    ):
        ...
        if diff_path and diff_threshold:
            ...
            added_constraints += (
                " AND abs(diff_temporal_distance.diff_mean) >= %s "
        ...
        if all_leg_sections:
            ...
            query = """SELECT from_stop_I, to_stop_I, coalesce(type, -1) AS type,
                         ...
                         GROUP BY from_stop_I, to_stop_I, type""" % (
                add_diff,
                str(target),
                added_constraints,
            )
    ...
    def _get_journey_legs_to_target_with_all_sections(self, target, added_constraint):
        ...
        df = DataFrame(
            [x for x in gen_pairs(orig_df.leg_stops.str.split(",").tolist())],
            index=[orig_df.type, orig_df.n_trips],
        ).stack()
    ...
    def get_origin_target_journey_legs(
        self,
        origin,
        target,
        start_time=None,
        ...
        df = read_sql_query(query, self.conn)
        if add_coordinates:
            df = self.g.add_coordinates_to_df(
                df, join_column="from_stop_I", lat_name="from_lat", lon_name="from_lon"
            )
            df = self.g.add_coordinates_to_df(
                df, join_column="to_stop_I", lat_name="to_lat", lon_name="to_lon"
            )
        return df

    def get_journey_routes_not_in_other_db(
        self,
        target,
        other_journey_conn,
        fastest_path=True,
        ...
        routes = other_journey_conn.execute(
            "SELECT DISTINCT route FROM journeys WHERE to_stop_I = %s" % (str(target),)
        ).fetchall()
        routes = [x[0] for x in routes]
        other_set = set(routes)
        these_routes = self.conn.execute(
            "SELECT DISTINCT route FROM journeys WHERE to_stop_I = %s" % (str(target),)
        ).fetchall()
        ...
        df = DataFrame(
            {"n_trips": df.groupby(["from_stop_I", "to_stop_I", "type"]).size()}
        ).reset_index()
        return df
    ...
    def get_upstream_stops_ratio(self, target, trough_stops, ratio):
        ...
        query = """SELECT stops.* FROM other.stops,
                    ...
                    WHERE stops.stop_I = q1.stop_I""".format(
            target=target, trough_stops=trough_stops, ratio=ratio
        )
        df = read_sql_query(query, self.conn)
        return df
