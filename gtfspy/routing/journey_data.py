...
class JourneyDataManager:
    def __init__(
        ...
        if routing_params.get("track_vehicle_legs", False) or self.routing_parameters.get(
    ...
    def import_journey_data_for_target_stop(
        self, target_stop_I, origin_stop_I_to_journey_labels, enforce_synchronous_writes=False
    ):
        """
        Parameters
        ----------
        ...
        if self.track_route:
            self._insert_journeys_with_route_into_db(
                origin_stop_I_to_journey_labels, target_stop=int(target_stop_I)
            )
        else:
    ...
    def _insert_journeys_with_route_into_db(self, stop_I_to_journey_labels, target_stop):
        ...
        for i, (origin_stop, labels) in enumerate(stop_I_to_journey_labels.items(), start=1):
            ...
            for label in labels:
                assert isinstance(label, LabelTimeAndRoute) or isinstance(
    ...
    def add_fastest_path_column(self):
        ...
        for target in self.get_targets_having_journeys():
            ...
            for origin in self.get_origins_having_journeys():
                ...
                all_labels = [
                    LabelTimeAndRoute(x[0], x[1], x[2], False) for x in all_trips
                ]  # putting journey_id as movement_duration
                all_fp_labels = compute_pareto_front(
                    all_labels, finalization=False, ignore_n_boardings=True
                )
                fastest_path_journey_ids.append(all_fp_labels)

            fastest_path_journey_ids = [
                (1, x.movement_duration) for sublist in fastest_path_journey_ids for x in sublist
            ]
            cur.executemany(
    ...
    def _journey_label_generator(self, destination_stop_Is=None, origin_stop_Is=None):
        ...
        for destination_stop_I in destination_stop_Is:
            ...
            sql = (
                "SELECT "
                + label_features
                + " FROM journeys WHERE to_stop_I = %s" % destination_stop_I
            )
    ...
    def get_node_profile_time_analyzer(self, target, origin, start_time_dep, end_time_dep):
        ...
        FROM journeys WHERE to_stop_I = %s AND from_stop_I = %s""" % (
            target,
            origin,
        )
        df = pd.read_sql_query(sql, self.conn)
    ...
    def get_node_profile_analyzer_time_and_veh_legs(
        self, target, origin, start_time_dep, end_time_dep
    ):
        sql = (
            """SELECT from_stop_I, to_stop_I, n_boardings, departure_time, arrival_time_target FROM journeys WHERE to_stop_I = %s AND from_stop_I = %s"""
            % (target, origin)
        )
        df = pd.read_sql_query(sql, self.conn)
        ...
        for journey in df.itertuples():
            ...
            journey_labels.append(
                LabelTimeWithBoardingsCount(
                    departure_time, arrival_time_target, n_boardings, first_leg_is_walk=float("nan")
                )
            )

        # This ought to be optimized...
        query = (
            """SELECT d, d_walk FROM stop_distances WHERE to_stop_I = %s AND from_stop_I = %s"""
            % (target, origin)
        )
        ...
        analyzer = NodeProfileAnalyzerTimeAndVehLegs(
            journey_labels, walk_duration, start_time_dep, end_time_dep  # walking time
        )
        return analyzer

    def __compute_travel_impedance_measure_dict(
        self, origin, target, journey_labels, analysis_start_time, analysis_end_time
    ):
        measure_summaries = {}
        ...
        for key, (value_no_next_journey, value_cutoff) in self.journey_properties.items():
            ...
            value_no_next_journey = (
                walking_duration if value_no_next_journey == _T_WALK_STR else value_no_next_journey
            )
    ...
    def compute_travel_impedance_measures_for_target(
        self, analysis_start_time, analysis_end_time, target, origins=None
    ):
        if origins is None:
        ...
        for origin, target, journey_labels in self._journey_label_generator([target], origins):
            measure_summary_dicts_for_pair = self.__compute_travel_impedance_measure_dict(
                origin, target, journey_labels, analysis_start_time, analysis_end_time
            )
            for measure in measure_summary_dicts_for_pair:
                measure_to_measure_summary_dicts[measure].append(
                    measure_summary_dicts_for_pair[measure]
                )
        return measure_to_measure_summary_dicts

    @timeit
    def compute_and_store_travel_impedance_measures(
        self,
        analysis_start_time,
        analysis_end_time,
        ...
        print(
            "TODO!, compute_and_store_travel_impedance_measures, "
    ...
    def initialize_comparison_tables(self, diff_db_path, before_db_tuple, after_db_tuple):
        ...
        self.diff_conn = attach_database(
            self.diff_conn, before_db_tuple[0], name=before_db_tuple[1]
        )
...
class DiffDataManager:
    ...
    def initialize_journey_comparison_tables(self, tables, before_db_tuple, after_db_tuple):
        ...
        for table in tables:
            self.conn.execute(
                "CREATE TABLE IF NOT EXISTS diff_" + table + "(from_stop_I INT, to_stop_I INT, "
    ...
    def get_table_as_dataframe(self, table_name, use_relative, target=None):
        if use_relative:
            query = (
                "SELECT from_stop_I, to_stop_I, rel_diff_min, rel_diff_max, rel_diff_median, rel_diff_mean FROM "
                + table_name
            )
        else:
            query = (
                "SELECT from_stop_I, to_stop_I, diff_min, diff_max, diff_median, diff_mean FROM "
                + table_name
            )
        if target:
    ...
    def get_temporal_distance_change_o_d_pairs(self, target, threshold):
        ...
        query = """SELECT from_stop_I FROM diff_temporal_distance
                    WHERE to_stop_I = %s AND abs(diff_mean) >= %s""" % (
            target,
            threshold,
        )
    ...
    def get_largest_component(self, target, threshold=180):
        query = """SELECT diff_pre_journey_wait_fp.from_stop_I AS stop_I, 
                    ...
                    AND diff_pre_journey_wait_fp.to_stop_I = %s""" % (
            target,
        )
        df = pd.read_sql_query(query, self.conn)
        df["max_component"] = df[
            ["pre_journey_wait", "in_vehicle_duration", "transfer_wait", "walking_duration"]
        ].idxmax(axis=1)
        df["max_value"] = df[
            ["pre_journey_wait", "in_vehicle_duration", "transfer_wait", "walking_duration"]
        ].max(axis=1)
...
class Parameters(object):
    ...
    def __contains__(self, key):
        val = self._conn.execute("SELECT value FROM parameters WHERE key=?", (key,)).fetchone()
